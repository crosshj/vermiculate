...
#define VERMICULATE_STANDALONE yes
...
static int hei = 1024, wid = 1280, speed = 100;
...
main (int argc, char **argv)
{
  ...
  // can't figure out a way around this without touching the original
  // maybe later
  // until then, I'm disabling it
  /*
  ...
  */
  ...
  for (argnum = 1; argnum < argc; argnum++)
    {
      ...
      else if (!strcmp (argv[argnum], "-random")){
      	
      	//printf("Number of strings = %d\n",(int) sizeof(sampleStrings) );
			//printf("%s\n",sampleStrings[0].str);
			//exit(1);
			//	sampleStrings[ranran]
			unsigned int ranran = random1 (sizeof (sampleStrings) / sizeof (sampleStrings[0]));
			printf("%d\n",ranran);
      	instring = sampleStrings[ranran].str;
      	speed = sampleStrings[ranran].speed;
  ...
  TEST("Right before vermiculate_main",0);
  vermiculate_main ();
  return 0;
}
