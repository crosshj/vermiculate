    ...
    if (bankt > 0) {
        ...
        switch (ch) {
            ...
            case 'O':
                ...
                break;
            case 'F':
                var fbank = getBank(); //TODO: should be a copy, yes???
                var fbankt = bankt;
                pickbank();
                threads = getThreads();
                bank=getBank();
                ...
                setThreads(threads); // TODO: because the threads we are working with are just copies, right???
                break;
            case 'L':
                bank=getBank();
                setForAllThreadsInBank(function(th){
                    th.prey = bank[bankc % bankt]; //TODO: is bankc ever set???
                });
                break;
            case 'R':
                ch = readkey();
